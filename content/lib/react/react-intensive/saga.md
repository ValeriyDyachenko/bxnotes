...
Если нужно обработать конкурентные или параллельные процессы, то блокирующий вызов не подойдет.
...
Для того, чтобы запустить гонку из нескольких процессов, вернуть результат первого выполненного процесса и отменить все остальные процессы, есть эффект ```race```.
